...
tickerRouter.get('/stream', async (req, res) => {
  ...
  const send = (event: TickerEvent) => {
    res.write(`data: ${JSON.stringify(event)}\n\n`);
  };

  async function fetchHighlights(sport: 'F1' | 'NFL') {
    try {
      const r = await fetch(`${base}/api/highlights?sport=${sport}`);
      const data = await r.json();
      const items = (Array.isArray(data) ? data : data.items) as Array<any>;
      for (const it of items.slice(0, 5)) {
        ...
        try {
          ...
          if (/youtube\.com|youtu\.be/.test(host)) source = 'YouTube';
          else source = host;
        } catch {
          source = null;
        }
  ...
  const pump = async () => {
    ...
    if (Date.now() - windowStart >= 120000) {
      ...
      sentIds.clear();
      for (const id of cut) sentIds.add(id);
    }
  };
