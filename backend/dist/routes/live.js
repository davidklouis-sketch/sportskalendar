...
let footballLiveCache = null;
let f1LiveCache = null;
...
async function fetchLiveFootballMatches() {
    ...
    try {
        ...
        const response = await fetchWithTimeout('https://api.football-data.org/v4/matches?status=LIVE', { headers });
        if (!response.ok) {
        ...
        const liveEntries = matches.slice(0, 10).map((match, index) => {
...
async function fetchF1LiveData() {
    // Check cache first
    ...
    try {
        ...
        const response = await fetchWithTimeout(`https://api.jolpi.ca/ergast/f1/${currentYear}/last/results.json`);
        if (!response.ok) {
        ...
        if (!race || !race.Results) {
            ...
            const standingsResponse = await fetchWithTimeout(`https://api.jolpi.ca/ergast/f1/${currentYear}/driverStandings.json`);
            if (standingsResponse.ok) {
                const standingsData = await standingsResponse.json();
                const standings = standingsData.MRData?.StandingsTable?.StandingsLists?.[0]?.DriverStandings || [];
                return standings.slice(0, 10).map((standing, index) => ({
                    position: index + 1,
        ...
        const f1Results = results.slice(0, 10).map((result, index) => ({
            position: index + 1,
...
async function getNextF1Race() {
    try {
        const currentYear = new Date().getFullYear();
        const response = await fetchWithTimeout(`https://api.jolpi.ca/ergast/f1/${currentYear}.json`);
        if (!response.ok) {
            return null;
        }
        const data = await response.json();
...
exports.liveRouter.get('/f1', async (_req, res) => {
    try {
...
exports.liveRouter.get('/nfl', (_req, res) => {
    try {
...
exports.liveRouter.get('/soccer', async (_req, res) => {
    try {
