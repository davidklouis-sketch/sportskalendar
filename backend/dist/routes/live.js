...
function randomShuffleStep(list) {
...
setInterval(() => {
    ...
    liveF1.forEach((e) => {
        if (typeof e.lap === 'number' && typeof e.totalLaps === 'number') {
            if (Math.random() < 0.3 && e.lap < e.totalLaps)
                e.lap = (e.lap || 0) + 1;
            if (typeof e.gapSec === 'number' && e.position > 1)
                e.gapSec = Math.max(0, (e.gapSec || 0) + (Math.random() * 0.6 - 0.3));
        }
    });
    liveSoccer.forEach((e) => {
        if (typeof e.minute === 'number' && e.minute < 90)
            e.minute += Math.random() < 0.5 ? 1 : 0;
    });
    liveNFL.forEach((e) => {
        if (typeof e.clock === 'string') {
            ...
            if (total <= 0) {
                e.quarter = Math.min(4, (e.quarter || 1) + 1);
                total = 15 * 60;
            }
...
exports.liveRouter.get('/f1', (_req, res) => {
...
exports.liveRouter.get('/soccer', (_req, res) => {
